...
import { initializeFavoritesDOM, updateFavoritesCount, refreshFavoritesDOMCache, } from './core/favorites';
import { setupLogoRefresh, createBookCard } from './ui/bookCard';
import { createLoadingCard, observeLoadingSentinel, preloadBookImages, } from './ui/loading';
 ...
 */
async function loadMoreBooks(count = CONFIG.LOAD_MORE_COUNT) {
    if (isLoadingMore.value || !cachedDOM.feed)
        return;
...
function setupIntersectionObserver() {
    if (observer)
        observer.disconnect();
    observer = observeLoadingSentinel(() => loadMoreBooks(CONFIG.LOAD_MORE_COUNT), isLoadingMore);
}
// ============ Initialization ============
/**
